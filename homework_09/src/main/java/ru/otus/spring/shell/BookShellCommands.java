...
public class BookShellCommands {
    ...
    static final String NO_COMMENTS_FOR_BOOK = "Нет комментариев для книги с идентификатором %d.";
    ...
    String updateBook(@ShellOption({"--id", "-I"}) long id,
        ...
        try{
            BookDTO bookDTO = new BookDTO(id, bookName, description, authorId, genreId);
            String bookStr = getBookInfo(bookService.update(bookDTO));
    ...
    String createBook(@ShellOption(value = {"--name", "-N"}) String bookName,
        ...
        try{
            BookDTO bookDTO = new BookDTO(null, bookName, description, authorId, genreId);
            String bookStr = getBookInfo(bookService.create(bookDTO));
    ...
    String getComments(@ShellOption({"--bookId", "-I"}) long bookId){
        try{
            List<BookComment> comments = commentService.findComments(bookId);
            if(CollectionUtils.isEmpty(comments)){
                return String.format(NO_COMMENTS_FOR_BOOK, bookId);
            } else {
    ...
    String addComment(@ShellOption({"--bookId", "-I"}) long bookId,
                      @ShellOption({"--userLogin", "-U"}) String userLogin,
                      @ShellOption({"--comment", "-C"}) String commentValue
    ){
        try{
            BookComment comment = commentService.addComment(new BookCommentDTO(bookId, userLogin, commentValue));
