...
class AuthenticationTest {
    ...
    @DisplayName("Должны успешно выполнить get-запросы для ранее авторизованного пользователя")
    @ParameterizedTest
    @ValueSource(strings = {"/about", "/admin", "/api/book/1"})
    void testSuccessAboutForAuthorizedUser(String path) throws Exception {
        mockMvc.perform(get(path)).andExpect(status().isOk());
    }

    @DisplayName("Должны успешно авторизоваться и открыть страницы about и admin")
    @ParameterizedTest
    @ValueSource(strings = {"/about", "/admin"})
    void testSuccessAuthorizeAndOpenAboutPage(String path) throws Exception {
        MockHttpSession session = getSession(ADMIN_LOGIN, ACTIVE_USER_PASS, true);
    ...
    private void verifyRejectActiveuserWrongPass(MockHttpServletRequestBuilder requestBuilder) throws Exception {
        ...
        MvcResult mvcResult = mockMvc.perform(formLogin().user(ADMIN_LOGIN).password("wrong_pass"))
