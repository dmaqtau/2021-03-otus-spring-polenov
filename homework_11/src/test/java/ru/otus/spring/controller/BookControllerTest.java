...
import ru.otus.spring.config.NoSecurityConfig;
...
@SpringBootTest(classes = {Main.class, NoSecurityConfig.class}, webEnvironment= SpringBootTest.WebEnvironment.RANDOM_PORT)
class BookControllerTest {
    ...
    private ResponseEntity<ErrorNotification> deleteBookByIdErr(Long bookId){
        return restTemplate.withBasicAuth("director","pepper").exchange(String.format("http://localhost:%d/api/book/%d", port, bookId), HttpMethod.DELETE, null, ErrorNotification.class);
    }
    ...
    private ResponseEntity<List> getAllBooks(){
        return this.restTemplate.getForEntity(String.format("http://localhost:%d/api/book", port), List.class);
    }
